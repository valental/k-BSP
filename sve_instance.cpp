...
/// Sluzi za testiranje svih mogucih instanca za razlicite k-ove i za svaku pojedinacno
/// ispisuje postotak kao i prosjek instanci za neki par n i k.
int main()
{
    ...
    for(int pom : nSIZE)
    {
        ...
        for(int k : k_from)
        {
            ...
            for(int i=0; i<problems; i++)
            {
                ...
                out << "Problem: " << i << "  MSTheuristic(%) = " << temp1 << "         multiswarm average(%) = " << temp2 << endl;
            }
            avg1 /= problems;
            avg2 /= problems;
            out << "MSTheuristic AVERAGE(%) = " << avg1 << " multiswarm AVERAGE(%) = " << avg2 << endl << endl << endl;
       }
